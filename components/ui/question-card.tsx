    ...
    <div className="w-full max-w-3xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <span className="px-3 py-1 rounded-lg bg-primary/20 text-primary text-sm font-medium">
            Questao {questaoNumber}/{totalQuestoes}
          </span>
          <span
            className={cn(
        ...
        {isTimed && timeRemaining !== undefined && (
          <div
            className={cn(
              ...
              timeRemaining < 30 ? 'bg-red-500/20 text-red-400' : 'bg-card text-muted-foreground'
            )}
          >
      ...
      <div className="h-1 bg-card rounded-full mb-8 overflow-hidden">
        <div
          ...
          style={{ width: `${(questaoNumber / totalQuestoes) * 100}%` }}
        />
      </div>
          ...
          return (
            <motion.button
              ...
              className={cn(
                ...
                showFeedback && isCorrectChoice && 'border-secondary bg-secondary/10',
                showFeedback && isSelected && !isCorrectChoice && 'border-red-500 bg-red-500/10',
                showFeedback && !isSelected && !isCorrectChoice && 'border-border opacity-50',
              ...
              <div className="flex items-start gap-4">
                <div
                  className={cn(
                    ...
                    !showFeedback && !isSelected && 'bg-card border border-border',
                    showFeedback && isCorrectChoice && 'bg-secondary text-white',
                    showFeedback && isSelected && !isCorrectChoice && 'bg-red-500 text-white'
                  )}
                >
                ...
                <span className="text-sm leading-relaxed pt-1">{alternativa}</span>
              </div>
            </motion.button>
          );
        })}
              ...
              <div className="flex items-center gap-2 mb-2">
                {isCorrect ? (
                  <>
                    ...
                    <span className="font-semibold text-secondary">Correto!</span>
                  </>
                ) : (
                  <>
                    <X className="w-5 h-5 text-red-400" />
                    <span className="font-semibold text-red-400">Incorreto</span>
                  </>
                )}
              </div>
