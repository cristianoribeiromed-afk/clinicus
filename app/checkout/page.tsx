      ...
      <main className="pt-24 pb-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial="hidden" animate="visible" variants={fadeInUp}>
            <h1 className="text-2xl md:text-3xl font-bold mb-2">Finalizar Assinatura</h1>
            <p className="text-muted-foreground mb-8">Revise seu pedido e escolha a forma de pagamento</p>
          </motion.div>
                ...
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {plan.interval === 'year' ? 'Cobranca anual' : 'Cobranca mensal'}
                    </p>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-border">
                  <div className="flex items-baseline justify-between">
                    ...
                    <div className="text-right">
                      ...
                      {plan.interval && (
                        <span className="text-sm text-muted-foreground">/{plan.interval === 'month' ? 'mes' : 'ano'}</span>
                      )}
                    </div>
                  </div>
                  {plan.discount && (
                    <p className="text-sm text-secondary text-right mt-1">{plan.discount}</p>
                  )}
                </div>
              </div>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className={`w-5 h-5 rounded-full border-2 ${
                        paymentMethod === 'pix' ? 'border-primary bg-primary' : 'border-border'
                      }`}>
                        {paymentMethod === 'pix' && <Check className="w-full h-full text-white p-0.5" />}
                      </div>
                    </div>
                  </button>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className={`w-5 h-5 rounded-full border-2 ${
                        ...
                        {paymentMethod === 'card' && <Check className="w-full h-full text-white p-0.5" />}
                      </div>
                    </div>
                  </button>
                </div>
