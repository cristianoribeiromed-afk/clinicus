...
function SimuladoContent() {
  ...
  const questoes: Questao[] = content?.questoes ? (content.questoes as Questao[]) : [];
    ...
    const fetchSimulado = async () => {
      try {
        ...
        if (contentData.tempo_por_questao && contentData.questoes) {
          setTempoRestante(contentData.tempo_por_questao * (contentData.questoes as Questao[]).length);
        }
      } catch (error) {
  ...
  const handleFinish = useCallback(
    async (finalRespostas?: number[]) => {
      ...
      if (content && profile) {
        ...
        const acertos = resp.reduce((acc, resp, idx) => {
          return acc + (resp === questoes[idx].gabarito ? 1 : 0);
        }, 0);
    ...
    [content, profile, respostas, questoes, startTime]
  );

  const calcularAcertos = () => {
    return respostas.reduce((acc, resp, idx) => acc + (resp === questoes[idx]?.gabarito ? 1 : 0), 0);
  };
  ...
  const handleRefazer = () => {
    ...
    if (content?.tempo_por_questao && content?.questoes) {
      setTempoRestante(content.tempo_por_questao * (content.questoes as Questao[]).length);
    }
    setIsRunning(true);
  };
    ...
    return (
      <AppLayout>
        <div className="p-4 lg:p-8 text-center">
          <h2 className="text-xl font-semibold mb-4">Simulado nao encontrado</h2>
          <Link href="/simulados">
    ...
    <AppLayout>
      <div className="p-4 lg:p-8 space-y-6">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          <Link href="/simulados" className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm text-primary font-medium">Simulado</span>
        ...
        <AnimatePresence mode="wait">
          {!hasAccess ? (
            <motion.div key="paywall" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center justify-center min-h-[400px]">
              <Paywall
                title="Simulado Premium"
                description="Este simulado exclusivo possui questoes selecionadas e gabarito comentado detalhado."
                ctaText="Assinar para Acessar"
              />
            </motion.div>
          ) : !isRunning && !showResult ? (
            <motion.div key="start" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center py-12">
              <div className="max-w-md mx-auto space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-muted-foreground">
                    {questoes.length} questoes | Tempo: {Math.round((questoes.length * (content.tempo_por_questao || 90)) / 60)} minutos
                  </p>
                </div>
                <Button onClick={handleStart} size="lg" className="bg-primary hover:bg-primary/90 text-white gap-2">
                  Iniciar Simulado
                </Button>
              </div>
            </motion.div>
          ) : showResult ? (
            <motion.div key="result" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="py-8">
              <ResultDisplay
            ...
            <motion.div key="question" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex justify-center">
              <QuestionCard
...
export default function SimuladoPage() {
  return (
    <Suspense fallback={<AppLayout><div className="p-8">Carregando...</div></AppLayout>}>
      <SimuladoContent />
    </Suspense>
  );
}
