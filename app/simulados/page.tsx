  ...
  return (
    <AppLayout>
      <div className="p-4 lg:p-8 space-y-6">
        ...
        <motion.div initial="hidden" animate="visible" variants={fadeInUp} className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground">Teste seus conhecimentos com questoes comentadas</p>
            </div>
          </div>
        </motion.div>
          ...
          filterOptions={{
            ciclo: {
              ...
              options: [
                ...
                { value: 'basico', label: 'Ciclo Basico' },
                { value: 'clinico', label: 'Ciclo Clinico' },
              ],
            },
            sort: {
              value: sort,
        ...
        {!isPremium && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 rounded-xl bg-secondary/10 border border-secondary/20">
            <div className="flex items-center gap-3">
              <Brain className="w-5 h-5 text-secondary" />
              <div className="flex-1">
                <p className="text-sm font-medium">Experimente o simulado demo gratis!</p>
                <p className="text-xs text-muted-foreground">10 questoes para conhecer a plataforma</p>
              </div>
        ...
        {isLoading ? (
          ...
          <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.05 } } }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        ...
        {!isPremium && filteredContents.filter((c) => c.premium).length > 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-6 rounded-xl bg-gradient-to-r from-primary/10 to-secondary/10 border border-primary/20">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="text-center sm:text-left">
                <h3 className="font-semibold">Desbloqueie todos os simulados</h3>
                <p className="text-sm text-muted-foreground">
                  {filteredContents.filter((c) => c.premium).length} simulados premium disponiveis
                </p>
              </div>
              <Link href="/planos">
